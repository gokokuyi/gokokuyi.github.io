"""
Module 6: 評価（Evals）— 演習ファイル
各ステップの # EDIT HERE! 部分を埋めてください。

実行方法:
    python module6_evals.py --step 1   # ルールベース評価（オフライン）
    python module6_evals.py --step 2   # LLM-as-judge（Azure OpenAI 必要）
    python module6_evals.py --step 3   # 回帰テスト（Azure OpenAI 必要）
"""
...
def evaluate_json_output(llm_output: str) -> dict:
    ...
    #
    # チェック3: priority の値が有効か確認
    # if "priority" in data:
    #     if data["priority"] not in EXPECTED_SCHEMA["priority_values"]:
    #         result["errors"].append(...)
    #
    # チェック4: action_items の件数確認
    # if "action_items" in data:
    #     if len(data["action_items"]) < EXPECTED_SCHEMA["action_items_min_count"]:
    #         result["errors"].append(...)
    #
    # すべてのチェックを通過した場合:
...
def llm_judge(client, model: str, question: str, answer: str) -> dict:
    ...
    # EDIT HERE! client.responses.create() を呼び出して judge_prompt を input に渡し、
    # 返ってきた JSON をパースして返してください
    #
    # ヒント:
    # try:
    #     response = client.responses.create(model=model, input=judge_prompt)
    #     text = response.output_text
    #     # コードブロックで囲まれている場合に JSON 部分だけ抽出する
...
def step2(config):
    ...
    for i, case in enumerate(test_cases, 1):
        ...
        if result:
            ...
            print("評価結果を取得できませんでした（llm_judge() を実装してください）")
...
def step3(config):
    ...
    if results["total"] > 0 and results["passed"] + results["failed"] > 0:
    ...
    if results["passed"] == 0 and results["failed"] == 0:
        print("\n⚠️  run_regression_test() を実装してください（# EDIT HERE! 部分）")


# ── メイン ────────────────────────────────────────────────────────────────────
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Module 6: 評価（Evals）演習"
    )
    parser.add_argument(
        "--step",
        type=int,
