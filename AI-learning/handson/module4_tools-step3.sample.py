#!/usr/bin/python
"""
Module 4: ツール利用 — Step 3: ツールパラメータの拡張 解答例
"""
import json
import textwrap
...
def search_axxx_group_documents(
    ...
    """Axxx Japanグループの社内文書をベクトル検索する（カテゴリフィルタ付き）"""
    ...
    tools = [
        {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    # Step 3 解答: category パラメータを追加
                    "category": {
                        "type": "string",
                        "enum": list(CATEGORY_TO_FILENAME.keys()),
                        "description": (
                            "検索対象のFAQカテゴリ。"
                            "general=全般, systems=システム, consulting=コンサルティング, "
                            "data=データ, ai=AI推進部。不明な場合は空文字列。"
                        ),
                    },
                },
