...
def load_vector_store(search_dir: str, client: OpenAI, reset: bool) -> dict[str, Any]:
    ...
    for resource in resources:
        ...
        try:
            # ══════════════════════════════════════════════════════════════
            # Step A: content の埋め込み表現を取得しましょう
            # ヒント: client.embeddings.create(input=content, model="text-embedding-3-small")
            #         レスポンスの .data[0].embedding がベクトルです
            # ══════════════════════════════════════════════════════════════
            embedding = None  # EDIT HERE!
...
def main(search_dir: str = "203_rag-chat", top_k: int = 3, reset: bool = False) -> None:
    ...
    # 対話履歴の読み込み
    messages = []
    path_to_chatlog = Path(search_dir) / "chatlog.jsonl"
    ...
    while True:
        ...
        # ══════════════════════════════════════════════════════════════
        # Step B: ユーザー入力の埋め込み表現を取得しましょう
        # ヒント: client.embeddings.create(input=user_input, model="text-embedding-3-small")
        #         結果は user_input_embedding という変数に代入してください
        # ══════════════════════════════════════════════════════════════
        user_input_embedding = None  # EDIT HERE!
        ...
        for id_ in vector_store:
            ...
            # Step C: コサイン類似度を計算しましょう
            # ヒント: calc_cosine_similarity(user_input_embedding, embedding_in_vector_store)
            cosine_similarity = None  # EDIT HERE!
        ...
        # ══════════════════════════════════════════════════════════════
        # Step D: ユーザー入力と検索結果を API に渡してレスポンスを取得しましょう
        # ヒント:
        #   1. messages に user_input_with_contexts を追加する
        #      messages = messages + [{"type": "message", "role": "user", "content": user_input_with_contexts}]
        #   2. client.responses.create(instructions=instructions, input=messages, **params) で呼び出す
        #   3. response.output_text を print し、assistant メッセージを messages に追加する
        # ══════════════════════════════════════════════════════════════
        # EDIT HERE!
