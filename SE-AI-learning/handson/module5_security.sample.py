"""
Module 5: セキュリティ & ガバナンス — 解答例ファイル
このファイルはすべての # EDIT HERE! 部分が実装済みです。
学習時に自分の実装と比較するために参照してください。

実行方法:
    python module5_security.sample.py --step 1   # PIIマスキング（オフライン）
    python module5_security.sample.py --step 2   # プロンプトインジェクション検出（オフライン）
    python module5_security.sample.py --step 3   # 安全なシステムプロンプト設計（Azure OpenAI 必要）
"""
...
def step1():
    ...
    if "[EMAIL]" in masked and "[PHONE_JP]" in masked:
        ...
        print("\n❌ mask_pii() 関数を確認してください")


# ═══════════════════════════════════════════════════════════════════════════════
# Step 2: プロンプトインジェクション検出（解答例）
# ═══════════════════════════════════════════════════════════════════════════════
...
def detect_injection(user_input: str) -> tuple[bool, list[str]]:
    ...
    for pattern in INJECTION_PATTERNS:
        # ✅ 解答: re.search で大文字小文字を区別せずにパターンを検索する
...
"""
    ...
    try:
        # ✅ 解答: safe_instructions をシステムプロンプトとして設定し、疑わしい入力を送信する
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Module 5: セキュリティ & ガバナンス 演習（解答例）"
    )
    parser.add_argument(
        "--step",
        type=int,
